...
prepare_trajectory_data <- function(pre_data = NULL, post_data = NULL,
  ...
  if (!is.null(post_data)) {
    ...
    out <- rbind(out, data.frame(
      id        = post_data[[id_var]],
      group     = as.character(post_data[[batch_var]]),
      age       = post_data$age,
      sex       = post_data$sex,
  ...
  # Fix panel order: Pre always left, Post always right.
  out$data_type <- factor(out$data_type,
...
create_trajectory_plot <- function(bin_summary, feature_name, output_dir,
                                   group_col = "batch", smooth_method = "loess",
                                   value_limits = NULL) {
  ...
  logger::log_info(paste0("  Saved: ", out_file))
  invisible(p)
}

plot_trajectories <- function(features, pre_data = NULL, post_data = NULL,
                              output_dir, batch_var = "batch", id_var = "id",
                              group_col = "batch", smooth_method = "loess",
                              age_bin_width = 5, fix_y_limits = TRUE) {
  dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

  for (feat in features) {
    logger::log_info(paste0("Plotting: ", feat))
    tryCatch({
    ...
    }, error = function(e) logger::log_info(paste0("  ERROR: ", e$message)))
  }
  invisible(NULL)
}