...
option_list <- list(
  ...
  make_option("--stratify_permutation",
    ...
    metavar = "TRUE/FALSE",
    help    = paste(
      "If TRUE, batch labels are permuted only within age x sex strata and",
      "strata containing a single batch are excluded.",
      "Required when batch is confounded with age (multi-cohort lifespan designs).",
      "FALSE gives an unconditional test; labels are still permuted at the",
      "subject level either way. [default: TRUE]"
    )),
  ...
  make_option("--thresh_skew_range",
    ...
    default = NULL,
    metavar = "N",
    help    = "[continuous] Absolute floor on the between-batch skewness range. Backstop only; the operative effect-size guard is the permutation p-value on the range. [default: 0.3]"),

  make_option("--thresh_kurt_range",
    type    = "numeric",
    default = NULL,
    metavar = "N",
    help    = "[continuous] Absolute floor on the between-batch excess kurtosis range. Backstop only; see --thresh_skew_range. [default: 0.5]"),
...
parser <- OptionParser(
  ...
  description = paste(
    ...
    "  Residualises for age (natural cubic spline), sex and batch, computes",
    "  skewness and excess kurtosis, and tests whether either differs between",
    "  batches. The result assigns a complexity tier and a candidate family",
    "  ordering matched to the support of the feature.",
    "",
    "  The batch test permutes labels within age-band x sex strata, at the",
    "  subject level, and drops strata holding a single batch -- an unconditional",
    "  permutation tests a null in which batches share an age composition they do",
    "  not have, so age effects are read as batch effects. The retained share of",
    "  the sample is reported per feature as frac_common. Pass",
    "  --stratify_permutation FALSE for the unconditional test.",
    "",
    ...
    "",
    "Permutation tests are seeded per feature from --seed, so results are",
    "reproducible, independent of --n_cores, and stable when re-running a subset",
    "with --one_feature.",
    "",
    "Examples:",
    "  gamlssHarmo diagnose --data data/raw/my_data.csv",
    "  gamlssHarmo diagnose --data data/raw/my_data.csv --discrete TRUE",
    "  gamlssHarmo diagnose --data data/raw/my_data.csv --n_perm 4999",
    "  gamlssHarmo diagnose --data data/raw/my_data.csv \\",
    "    --stratify_permutation FALSE --suffix _uncond",
    "  gamlssHarmo diagnose --data data/raw/my_data.csv \\",
